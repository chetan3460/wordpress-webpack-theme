...
<section
    ...
    class="list_hover_v2  py-[6.25rem] px-16 max-xl:px-5 max-md:py-12 max-container fade-in  w-full  bg-[center_center] bg-no-repeat bg-cover bg-fixed"
    ...
    <div class="interactive-showcase">
        ...
        <div
            ...
            <h3
                ...
                <span
                    class="showcase__subtitle text-[14px] leading-[1.5em] inline-block relative w-[44px] min-w-fit text-center self-start mr-[24px] px-[6px] py-[3px] border border-[#7b7b7b] text-[#7b7b7b] rounded-[30px] transition-all duration-[0.4s] transform translate-y-[0.3em] flex-shrink-0">
                    0<?= $key + 1; ?></span>
                <span class=" title text-white transform skew-0 transition-all duration-700 md:mt-0 mt-4">
                    <?= $title; ?></span>
            </h3>
            <?php endif; ?>
            ...
            <div
                class="showcase__image interactive absolute h-auto   origin-[top_center] top-2/4 max-lg:hidden  left-0 transform -translate-y-1/2 z-[4] opacity-0 group-hover:opacity-100 pointer-events-none overflow-hidden transition-all duration-[0.45s] ml-[20px]">
                <img data-src="<?= $thumbnail['url'] ?>" alt="block image"
                    class=" lazy-image transform -translate-x-full group-hover:translate-x-0 transition-all duration-[0.8s] h-[140px]" />
            </div>
