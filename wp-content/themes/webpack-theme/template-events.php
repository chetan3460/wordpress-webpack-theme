...
<?php if (have_posts()) : ?>
    ...
    $postType = 'events';
    $taxonomyHandle = 'events_category';
    ?>

        <!-- Page banner -->
        <?php include(locate_template('template-parts/parts/short_banner.php', false, false)); ?>

        <!-- Event Latest listing -->
        <section class="event-list-block-wrap common-listing py-[6.25rem] max-md:py-12 px-16 max-xl:px-5  fade-in" data-type="<?php echo $postType; ?>">
            <div class="max-container">
                ...
                <?php if ($response['success'] && $response['data']->have_posts()) { ?>
                    <div class="mt-16 max-md:mt-10 ">
                        <div class="list-item-container flex gap-10 max-md:gap-7 flex-col">
                            <?php $query = $response['data'];
                                while ($query->have_posts()) : $query->the_post();
                                    get_template_part('template-parts/parts/events_card', 'post');
                                endwhile;
                                wp_reset_postdata();
                            ?>
                        </div>
